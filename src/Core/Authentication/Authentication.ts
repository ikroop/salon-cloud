...
export class Authentication implements AuthenticationBehavior {
   ...
   /**
    ...
    /**
     * signUpWithAutoGeneratedPassword
     * Sign Up with username, only for customer & employee in salon.
     * @param : 
    ...
    public async signUpWithAutoGeneratedPassword(username: string): Promise<SalonCloudResponse<any>> {
      ...
      *     - SalonCloudResponse: code 403, error = 'InvalidToken'  
      *     - SalonCloudResponse: code 200, data = { _id: string, username: string, status: boolean }
      * @memberOf Authentication
      */
