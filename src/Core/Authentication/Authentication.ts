...
export class Authentication implements AuthenticationBehavior {
     ...
     * @memberOf Authentication
     */
    public async signUpWithAutoGeneratedPassword(username: string): Promise<SalonCloudResponse<UserToken>> {
        var response: SalonCloudResponse<any> = {
            code: undefined,
            err: undefined,
            data: undefined
        ...
        if (registerProcess.err) {
            ...
            let signinData: SalonCloudResponse<UserToken> = await this.signInWithUsernameAndPassword(username, randomPasswordString);
