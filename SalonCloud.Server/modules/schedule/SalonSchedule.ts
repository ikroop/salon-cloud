...
export class SalonSchedule extends Schedule {
    ...
    protected async checkDailySchedule(dailySchedule: DailyDayData){
        ...
        var result = await DailyScheduleModel.findOne({salon_id: this.salonId, employee_id: null, "day.date": dailySchedule._id}).exec( function(err, docs){
            if(err){
    ...
    protected async checkWeeklySchedule(){
        ...
        var result = await WeeklyScheduleModel.findOne({ salon_id: this.salonId, employee_id: null}).exec( function(err, docs){
            if(err){
                return returnResult.err = err;
            }else if(docs){
                return returnResult.data = true;
    ...
    protected async updateDailySchedule(dailySchedule: DailyDayData) {
        ...
        var docsFound = await DailyScheduleModel.findOne({salon_id:this.salonId, employee_id: null}).exec();
    ...
    protected async updateWeeklySchedule(weeklyScheduleList: [WeeklyDayData]){
        ...
        var docsFound = await WeeklyScheduleModel.findOne({salon_id:this.salonId, employee_id: null}).exec();
